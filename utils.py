...
class CustomisedDLE(DistributedLearningEngine):
    def __init__(self, net, dataloader, max_norm=0, num_classes=117, **kwargs):
        ...
        self.max_norm = max_norm          #这个参数是什么意思哦
        self.num_classes = num_classes

    #重写基类的每个iteration 的动作
    ...
    def test_hico(self, dataloader):
        ...
        #好像测试的batch size 就是1 从代码中推断的话
        for batch in tqdm(dataloader):
            inputs = pocket.ops.relocate_to_cuda(batch[0])
            #print(type(inputs),len(inputs),type(inputs[0]))
            output = net(inputs)
            ...
            #print(output.keys()) #['boxes', 'pairing', 'scores', 'labels', 'objects', 'attn_maps', 'size']
            ...
            verbs = output['labels']
            interactions = conversion[objects, verbs]    #把物体和动词转换为交互
            '''
            temp_res={
                'boxes':boxes.tolist(),'boxes_h':boxes_h.tolist(),'boxes_o':boxes_o.tolist(),'pairing':output['pairing'].tolist(),
                'objects':objects.tolist(),'scores':scores.tolist(),'verbs':verbs.tolist(),'interactions':interactions.tolist()
            }
            store_result.append(temp_res)
            with open('detection_test_result.json','w') as f:
                json.dump(store_result,f)
            '''
