...
from tsce import build_detector
...
def main(rank, args):
    ...
    tsce = build_detector(args, object_to_target)

    if os.path.exists(args.resume):
        print(f"=> Rank {rank}: continue from saved checkpoint {args.resume}")
        checkpoint = torch.load(args.resume, map_location='cpu')
        tsce.load_state_dict(checkpoint['model_state_dict'])
    else:
    ...
    for p in tsce.detector.parameters():
        p.requires_grad = False

    param_dicts = [{
        "params": [p for n, p in tsce.named_parameters()
